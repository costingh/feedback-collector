		...
		>
			<Avatar>
				...
				<AvatarFallback className="bg-gray-200 text-gray-400">
					{selectedTestimonial?.name?.slice(0, 2) || 'N/A'}
				</AvatarFallback>
			</Avatar>
			<p
				className="text-gray-900 text-center font-normal text-[15px] mt-2"
				style={{ color: widget?.primaryTextColor }}
			>
				{selectedTestimonial?.message?.length > maxCharactersToShow &&
				!isExpanded(selectedTestimonial?.id)
				...
				{!selectedTestimonial?.message &&
					'Here it will be the testimonial content'}
			</p>
			...
			<span
				className="text-gray-400 font-normal text-[12px]"
			...
			>
				{selectedTestimonial?.jobTitle || 'Sales Manager'}
			</span>
		</div>
	)
}
