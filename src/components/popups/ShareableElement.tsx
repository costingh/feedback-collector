			...
			<div className="top w-full h-[170px] bg-gray-100 flex items-center justify-center overflow-hidden">
				...
				{type == 'rolling_wall' && (
					<div style={{ transform: 'scale(0.4)' }}>
						<RollingWall
							transition={true}
							testimonials={testimonialsMock}
							widget={getMockWidget('rolling_wall')}
						/>
					</div>
				)}
				...
				{type == 'rating_badge' && (
					<div>
						<RatingBadge
							transition={true}
							widget={getMockWidget('rating_badge')}
						/>
					</div>
				)}
				...
				{type == 'social_star' && (
					<div>
						<SocialStar
							...
							numberOfReviews={1041}
							widget={getMockWidget('social_star')}
							style={{ transform: 'scale(0.5)', width: '500px' }}
						/>
					</div>
				)}
				...
				{type == 'hero_quotes' && (
					<div>
						<HeroQuotes
							transition={true}
							widget={getMockWidget('hero_quotes')}
				...
				{type == 'minimalist_review' && (
					<div>
						<MinimalistReview
							review={testimonialsMock[0]}
							widget={getMockWidget('minimalist_review')}
							style={{ transform: 'scale(0.6)' }}
						/>
					</div>
				)}
