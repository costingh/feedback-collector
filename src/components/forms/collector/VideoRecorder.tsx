...
export default function VideoRecorder({ onVideoChange, isRequired = false }: VideoRecorderProps) {
	...
	const startRecording = () => {
		...
		recorder.onstop = () => {
			...
			const videoFile = new File([completeBlob], "video.webm", { type: "video/webm" });
				...
				<DropdownMenu>
					...
					<DropdownMenuContent>
						{availableMicrophones.map((microphone) => (
							...
							>
								{microphone.label || `Microphone ${microphone.deviceId.slice(0, 5)}`}
							</DropdownMenuItem>
						))}
					</DropdownMenuContent>
				</DropdownMenu>
