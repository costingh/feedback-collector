...
export default function TopCountriesList({ topCountries }: { topCountries: any[] }) {
	// Ensure there are always 5 rows (fill with empty placeholders if needed)
	const displayedCountries = [...topCountries, ...Array(5 - topCountries.length).fill(null)].slice(0, 5);
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Most traffic in the last month</CardDescription>
			</CardHeader>
			<CardContent>
						...
						country ? (
							<div key={country.country} className="flex items-center space-x-4">
								...
								<div className="flex-1">
									...
									<Progress value={(country.visits / maxVisits) * 100} />
								</div>
