...
export default function TopEngagingFormsList({ forms }: { forms: any[] }) {
	...
	const maxEngagement = Math.max(...forms.map((form) => form.interactionRate), 1);

	return (
		<Card>
			<CardHeader>
				<CardTitle>⚡ Top 5 Most Engaging Forms</CardTitle>
				<CardDescription>Highest interaction rate (views vs. submissions)</CardDescription>
			</CardHeader>
			<CardContent>
						...
						form ? (
							<div key={form.formName} className="flex items-center space-x-4">
								...
								<div className="flex-1">
									...
									<Progress value={(form.interactionRate / maxEngagement) * 100} />
								</div>

								{/* Interaction Rate */}
								<span className="text-gray-600">{form.interactionRate.toFixed(1)}%</span>
							</div>
						) : (
