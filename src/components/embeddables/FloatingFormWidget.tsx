...
export const FloatingFormWidget = ({
	children,
}: {
		...
		<>
			...
			>
				...
				<div className="relative z-20 flex h-full w-full items-center justify-center rounded-full purple-background !border-[2px] !border-[rgb(89,76,255)]">
					{!isOpen ? (
						<MegaphoneLoud20RegularIcon className="w-5 h-5 text-white" />
					) : (
						<CloseIconX className="w-5 h-5 text-white" />
					)}
				</div>
			</div>
			...
			<div
				className={clsx(
					...
					isOpen
						...
						: 'hidden animate-slide-up-reverse',
				)}
			>
				{children}
			</div>
