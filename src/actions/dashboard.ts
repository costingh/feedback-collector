...
export const getDashboardStats = async (workspaceId: string) => {
	...
	try {
		...
		if (!userInfo)
			return { status: 403, data: 'User not found', form: null }

		const [workspacesCount, forms, widgetsCount, testimonialsCount] =
			await Promise.all([
				...
				client.formResponse.count({
					where: { form: { userId: user.id, workspaceId } },
				}),
			])
		...
		const formAnalytics =
			formIds.length > 0
				? await client.formAnalytics.groupBy({
		...
		const totalVisits = formAnalytics.reduce(
			(sum, metric) => sum + (metric._sum.value || 0),
			0,
		)
		...
		const transformedData = formsWithMetrics.map((form: any) => {
			const viewMetric =
				form.metrics.find((metric: any) => metric.actionType === 'view')
					?.total || 0
			const completionMetric =
				form.metrics.find(
					(metric: any) => metric.actionType === 'completion',
				)?.total || 0
			const interactionMetric =
				form.metrics.find(
					(metric: any) => metric.actionType === 'interaction',
				)?.total || 0

			// Calculate interaction rate (total_interactions / total_views) * 100
			const interactionRate =
				viewMetric > 0 ? (interactionMetric / viewMetric) * 100 : 0
