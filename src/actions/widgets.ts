	...
	try {
		...
		if (widget) {
			return {
				...
				allTestimonialsIds: allTestimonials.map(
					(t: { id: string }) => t.id,
				),
				pagination: {
...
export const updateWidget = async (
	...
	try {
		...
		const currentTestimonialsIds = currentTestimonials.testimonials.map(
		...
		return {
			status: 200,
			data: 'Widget updated successfully',
			updatedWidget,
		}
	} catch (error) {
		return { status: 400 }
...
export const customizeWidget = async (
	widgetId: string,
	description: string,
