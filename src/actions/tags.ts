...
export const createTag = async ({
	...
	try {
		...
		if (!user || !workspaceId)
			return { status: 404, data: 'User or workspace was not found' }
...
export const deleteTag = async (tagId: string) => {
	try {
		...
		if (!user || !tagId)
			return { status: 404, data: 'User or tag was not found' }
		...
		if (response?.id) {
			...
			return {
				status: 400,
				data: 'Tag could not been deleted. Maybe it was created by another user in the same workspace you work.',
			}
	} catch (error) {
		console.log('Error creating tag ', error)
